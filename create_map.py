...
def create_map(results_1km, enhanced_data=None):
    ...
    .quiz-score, .quiz-streak, .quiz-best {{
        background: #f0f0f0;
        padding: 6px 12px;
        border-radius: 15px;
        font-weight: 600;
    ...
    body.dark-mode .quiz-score, body.dark-mode .quiz-best {{
        background: var(--border-dark);
    }}

    .quiz-content.shake-correct {{
        animation: shakeCorrect 0.3s ease;
    }}

    .quiz-content.shake-incorrect {{
    ...
    <div class="shortcuts-panel" id="shortcutsPanel">
        ...
        <div class="shortcut-item"><span>Take Quiz</span><kbd>Q</kbd></div>
    ...
    <div class="quiz-modal" id="quizModal">
        <div class="quiz-content">
            <button class="quiz-close" id="closeQuiz">&times;</button>
            <div class="quiz-header">
                <h2>&#x1F687; Transport Quiz</h2>
                <div class="quiz-stats">
                    <div class="quiz-score">Score: <span id="quizScore">0</span>/<span id="quizTotal">0</span></div>
                    <div class="quiz-streak">Streak: <span id="quizStreak">0</span> &#x1F525;</div>
                    <div class="quiz-best">Best: <span id="quizBestStreak">0</span></div>
                </div>
            </div>
            <div class="quiz-question" id="quizQuestion">Loading question...</div>
            <div class="quiz-options" id="quizOptions"></div>
            <div class="quiz-feedback" id="quizFeedback"></div>
            <button class="quiz-next" id="nextQuestion" style="display:none;">Next Question</button>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {{
        ...
        function initializeWhenReady() {{
            ...
            var quizTotal = 0;
            var quizStreak = 0;
            var quizBestStreak = parseInt(localStorage.getItem('tflQuizBestStreak') || '0');
            var currentQuestion = null;

            function updateStreakDisplay() {{
                var streakEl = document.getElementById('quizStreak');
                var bestStreakEl = document.getElementById('quizBestStreak');
                if (streakEl) {{
                    streakEl.textContent = quizStreak;
                    if (quizStreak >= 3) {{
                        streakEl.parentElement.classList.add('on-fire');
                    }} else {{
                        streakEl.parentElement.classList.remove('on-fire');
                    }}
                }}
                if (bestStreakEl) {{
                    bestStreakEl.textContent = quizBestStreak;
                }}
            }}
            ...
            function generateQuestion() {{
                ...
                var type = questionTypes[Math.floor(Math.random() * questionTypes.length)];
                var validStations = stations.filter(function(s) {{ return s.population_1km > 0 && s.daily_usage > 0; }});

                if (type === 'highest_population') {{
                    var options = [];
                    while (options.length < 4) {{
                        var s = validStations[Math.floor(Math.random() * validStations.length)];
                        if (!options.find(function(o) {{ return o.name === s.name; }})) {{
                            options.push(s);
                        }}
                    }}
                    options.sort(function(a, b) {{ return b.population_1km - a.population_1km; }});
                    var correct = options[0];
                    options = shuffleArray(options);

                    return {{
                        question: 'Which station has the HIGHEST population within 1km?',
                        options: options.map(function(s) {{ return s.name; }}),
                        correct: correct.name,
                        explanation: correct.name + ' has ' + formatNumber(correct.population_1km) + ' people within 1km!'
                    }};

                }} else if (type === 'highest_usage') {{
                    var options = [];
                    while (options.length < 4) {{
                        var s = validStations[Math.floor(Math.random() * validStations.length)];
                        if (!options.find(function(o) {{ return o.name === s.name; }})) {{
                    ...
                    options.sort(function(a, b) {{ return b.daily_usage - a.daily_usage; }});
                    var correct = options[0];
                    options = shuffleArray(options);

                    return {{
                        question: 'Which station has the HIGHEST daily usage?',
                        options: options.map(function(s) {{ return s.name; }}),
                        correct: correct.name,
                        explanation: correct.name + ' sees ' + formatNumber(correct.daily_usage) + ' passengers daily!'
                    }};

                }} else if (type === 'station_type') {{
                    var s = validStations[Math.floor(Math.random() * validStations.length)];
                    var types = ['commuter_hub', 'residential_hub', 'balanced', 'low_activity'];
                    var typeLabels = {{
                        'commuter_hub': 'Commuter Hub',
                        'residential_hub': 'Residential Area',
                        'balanced': 'Balanced',
                    ...
                    return {{
                        question: 'What type of station is ' + s.name + '?',
                        options: types.map(function(t) {{ return typeLabels[t]; }}),
                        correct: typeLabels[s.station_type] || 'Low Activity',
                        explanation: s.name + ' is classified as ' + (typeLabels[s.station_type] || 'Low Activity') + ' based on its population-to-usage ratio.'
                    }};

                }} else if (type === 'comparison') {{
                    var s1 = validStations[Math.floor(Math.random() * validStations.length)];
                    var s2 = validStations[Math.floor(Math.random() * validStations.length)];
                    while (s2.name === s1.name) {{
                        s2 = validStations[Math.floor(Math.random() * validStations.length)];
                    }}
                    var metric = Math.random() > 0.5 ? 'population_1km' : 'daily_usage';
                    var metricLabel = metric === 'population_1km' ? 'population' : 'daily usage';
                    var higher = s1[metric] > s2[metric] ? s1 : s2;

                    return {{
                        question: 'Which station has higher ' + metricLabel + ': ' + s1.name + ' or ' + s2.name + '?',
                        options: [s1.name, s2.name],
                        correct: higher.name,
                        explanation: higher.name + ' has ' + formatNumber(higher[metric]) + ' vs ' + formatNumber((higher === s1 ? s2 : s1)[metric]) + '!'
                    }};

                }} else {{
                    var lines = Object.keys(lineStats);
                    var line = lines[Math.floor(Math.random() * lines.length)];
                    var lineStations = stations.filter(function(s) {{ return s.lines.includes(line); }});
                    var correct = lineStations.length;
                    var options = [correct];
                    while (options.length < 4) {{
                        var opt = correct + Math.floor(Math.random() * 20) - 10;
                    ...
                    return {{
                        question: 'How many stations are on the ' + lineStats[line].name + ' line?',
                        options: options.map(function(n) {{ return n.toString(); }}),
                        correct: correct.toString(),
                        explanation: 'The ' + lineStats[line].name + ' line has ' + correct + ' stations!'
                    }};
                }}
            }}
            ...
            function showQuestion() {{
                ...
                quizOptions.innerHTML = currentQuestion.options.map(function(opt) {{
                    return '<div class="quiz-option" data-answer="' + opt + '">' + opt + '</div>';
                }}).join('');
                quizFeedback.classList.remove('visible', 'correct', 'incorrect');
                nextQuestion.style.display = 'none';

                quizOptions.querySelectorAll('.quiz-option').forEach(function(opt) {{
            ...
            function checkAnswer(answer) {{
                ...
                quizOptions.querySelectorAll('.quiz-option').forEach(function(opt) {{
                    ...
                    if (opt.dataset.answer === currentQuestion.correct) {{
                ...
                if (answer === currentQuestion.correct) {{
                    ...
                    if (quizStreak > quizBestStreak) {{
                        ...
                        localStorage.setItem('tflQuizBestStreak', quizBestStreak.toString());
                    }}
                    quizScoreEl.textContent = quizScore;
                    var streakBonus = quizStreak >= 3 ? ' \\uD83D\\uDD25 ' + quizStreak + ' in a row!' : '';
                    quizFeedback.textContent = 'Correct! ' + currentQuestion.explanation + streakBonus;
                    quizFeedback.classList.add('visible', 'correct');
                    quizContent.classList.add('shake-correct');
                    setTimeout(function() {{ quizContent.classList.remove('shake-correct'); }}, 300);
                }} else {{
                    quizStreak = 0;
                    quizFeedback.textContent = 'Not quite. ' + currentQuestion.explanation;
                    quizFeedback.classList.add('visible', 'incorrect');
                    quizContent.classList.add('shake-incorrect');
                    setTimeout(function() {{ quizContent.classList.remove('shake-incorrect'); }}, 300);
                }}
                updateStreakDisplay();
            ...
            if (quizBtn) {{
                quizBtn.addEventListener('click', function() {{
                    ...
                    quizTotal = 0;
                    quizStreak = 0;
                    quizBestStreak = parseInt(localStorage.getItem('tflQuizBestStreak') || '0');
            ...
            document.addEventListener('keydown', function(e) {{
                ...
                switch(e.key) {{
                    ...
                    case 'q':
                    case 'Q':
                        if (quizModal && !quizModal.classList.contains('visible')) {{
                            quizModal.classList.add('visible');
                            quizScore = 0;
                            quizTotal = 0;
                            quizStreak = 0;
    ...
    ">
        ...
        <div class="export-controls">
            <h4>Export & Share:</h4>
            <button id="exportPNGBtn" class="export-btn" title="Press E for quick export">&#x1F4F7; Download Map as PNG</button>
            <button id="shareBtn" class="export-btn" style="background:#e3f2fd;border-color:#90caf9;" title="Share your current view with classmates">&#x1F517; Copy Link to Share</button>
            <button id="quizBtn" class="export-btn" style="background:#fce4ec;border-color:#f48fb1;" title="Press Q for quick access">&#x1F3AE; Take the Quiz</button>
        </div>
